        ...
        <div class="mr-3">
            ...
            <c:choose>
                <c:when test="${sessionScope.currentUser.role == 'STUDENT'}">
                    <a href="${pageContext.request.contextPath}/student.jsp" class="btn btn-outline-primary position-absolute start-0">Back</a>
                </c:when>
                <c:when test="${sessionScope.currentUser.role == 'INSTRUCTOR'}">
                    <a href="${pageContext.request.contextPath}/instructor.jsp" class="btn btn-outline-primary position-absolute start-0">Back</a>
                </c:when>
                <c:when test="${sessionScope.currentUser.role == 'ADMIN'}">
                    <a href="${pageContext.request.contextPath}/admin.jsp" class="btn btn-outline-primary position-absolute start-0">Back</a>
                </c:when>
                <c:otherwise>
